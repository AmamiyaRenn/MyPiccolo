...
namespace Piccolo
{
    void metaExample()
    {
        ...
        std::fstream out_put("out.json", std::ios::out);
        ...
        for (int i = 0; i < fields_count; ++i)
        {
            ...
            std::cout << filed_accesser.getFieldTypeName() << " " << filed_accesser.getFieldName() << " "
                      << static_cast<char*>(filed_accesser.get(meta.m_instance)) << std::endl;
            if (filed_accesser.isArrayType())
            {
                ...
                if (Reflection::TypeMeta::newArrayAccessorFromName(filed_accesser.getFieldTypeName(), array_accesser))
                {
                    ...
                    auto  type_meta_item = Reflection::TypeMeta::newMetaFromName(array_accesser.getElementTypeName());
                    for (int index = 0; index < count; ++index)
                    {
                        std::cout << ":L:" << index
                                  << ":R:" << static_cast<int*>(array_accesser.get(index, field_instance)) << std::endl;
                        ;
                    }
                }
            }
